...
from ultralytics.nn.modules.CascadeHead import OBB_CascadeHead
import contextlib
import pickle
import re
import types
...
from ultralytics.nn.cascade_detect import CascadeROIOBB
...
#
#     # 打印模型结构信息
#     print(f"[DEBUG] 模型结构长度: backbone={len(d['backbone'])}, head={len(d['head'])}")
#     print(f"[DEBUG] 初始ch值: {ch}")
#
#     for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
#         print(f"\n[DEBUG] 处理第{i}层: f={f}, n={n}, m={m}, args={args}")
#         print(f"[DEBUG] 当前ch列表: {ch}, 长度: {len(ch)}")
#
#         m = (
...
#             print(f"[DEBUG] 进入base_modules分支")
#             c1, c2 = ch[f], args[0]
#             print(f"[DEBUG] base_modules - f={f}, c1={c1}, c2={c2}")
...
#                 legacy = False
#         elif m is AIFI:
#             print(f"[DEBUG] 进入AIFI分支")
#             args = [ch[f], *args]
#         elif m in frozenset({HGStem, HGBlock}):
#             print(f"[DEBUG] 进入HG分支")
...
#             print(f"[DEBUG] 进入ResNetLayer分支")
#             c2 = args[1] if args[3] else args[1] * 4
#         elif m is torch.nn.BatchNorm2d:
#             print(f"[DEBUG] 进入BatchNorm2d分支")
#             args = [ch[f]]
#         elif m is Concat:
#             print(f"[DEBUG] 进入Concat分支")
#             if isinstance(f, int):  # f 是单个层索引
#                 c2 = ch[f]
#                 ch_f = [ch[f]]
#             elif isinstance(f, (list, tuple)):  # f 是多个层索引
#                 c2 = sum(ch[x] for x in f)
#                 ch_f = [ch[x] for x in f]
#             else:
#                 raise TypeError(f"Concat 层 f 类型错误: {type(f)}, 值={f}")
#             print(f"[DEBUG] Concat - f={f}, ch_f={ch_f}, c2={c2}")
#         elif m in frozenset(
#                 {Detect,CascadeROIDetect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
#         ):
#             print(f"[DEBUG] 进入检测头分支: {m}")
#             args.append([ch[x] for x in f])
#             print(f"[DEBUG] Detect Head 输入层 f={f}, ch={ch}, 选中的ch={[ch[x] for x in f]}")
#             print(f"[DEBUG] Detect Head args={args}, nc={nc}, 当前c2={c2}")
#
...
#             print(f"[DEBUG] 进入RTDETRDecoder分支")
#             args.insert(1, [ch[x] for x in f])
#         elif m is CBLinear:
#             print(f"[DEBUG] 进入CBLinear分支")
#             c2 = args[0]
#             c1 = ch[f]
#             args = [c1, c2, *args[1:]]
#         elif m is CBFuse:
#             print(f"[DEBUG] 进入CBFuse分支")
#             c2 = ch[f[-1]]
#         elif m in frozenset({TorchVision, Index}):
#             print(f"[DEBUG] 进入TorchVision/Index分支")
#             c2 = args[0]
#             c1 = ch[f]
#             args = [*args[1:]]
#         else:
#             print(f"[DEBUG] 进入默认分支")
#             print(f"[DEBUG] 默认分支 - f={f}, ch长度={len(ch)}, 尝试访问ch[{f}]")
#             if f >= len(ch):
#                 print(f"[ERROR] 索引越界: f={f}, ch长度={len(ch)}")
#                 print(f"[ERROR] 当前层信息: i={i}, m={m}, args={args}")
#                 raise IndexError(f"索引{f}超出ch列表范围(长度{len(ch)})")
#             c2 = ch[f]
#             print(f"[DEBUG] 默认分支 - c2={c2}")
#
...
#             ch = []
#         ch.append(c2)
#         print(f"[DEBUG] 第{i}层处理完成，添加c2={c2}到ch列表")
#         print(f"[DEBUG] 更新后ch列表: {ch}")
#
#     print(f"[DEBUG] 模型解析完成，总层数: {len(layers)}")
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, OBB_CascadeHead,WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
            args.append([ch[x] for x in f] if isinstance(f, list) else ch[f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB,OBB_CascadeHead}:
                m.legacy = legacy
        elif m is RTDETRDecoder:  # special case, channels arg must be passed in index 1
            args.insert(1, [ch[x] for x in f])
        elif m is CBLinear:
