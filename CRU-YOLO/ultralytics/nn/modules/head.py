...
class Detect(nn.Module):
    ...
    def forward(self, x: list[torch.Tensor]) -> list[torch.Tensor] | tuple:
        ...
        if not hasattr(self, "_debug_forward"):
            print(f"\n[DEBUG] Detect.forward() mode = {'train' if self.training else 'eval'}")
            print(f"[DEBUG] 输入层数: {len(x)}")
            for i, xi in enumerate(x):
                print(f"  x[{i}].shape={tuple(xi.shape)} device={xi.device}")
    ...
    def _inference(self, x: list[torch.Tensor]) -> torch.Tensor:
        ...
        """
        if not hasattr(self, "_debug_infer"):
            print("\n[DEBUG] ====== Detect._inference() ======")
            print(f"type(x)={type(x)}")
            if isinstance(x, list):
                for i, xi in enumerate(x):
                    print(f"  x[{i}].shape={tuple(xi.shape)} device={xi.device}")
            print(f"self.nl={self.nl}, self.stride={self.stride}")
            print(f"hasattr(self, 'angle')={hasattr(self, 'angle')}")
            if hasattr(self, 'angle'):
                print(f"self.angle.shape={tuple(self.angle.shape)} dtype={self.angle.dtype} device={self.angle.device}")
            else:
                print("⚠️ self.angle 不存在")
            self._debug_infer = True
        # Inference path
